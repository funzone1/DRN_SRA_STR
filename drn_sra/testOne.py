...
def test(args):
    ...
    if args.arch == 'resnet18':
        ...
        model = models.drn_a_18(
            ...
            scale=args.scale,
            rec_cfg=rec_cfg)
    elif args.arch == 'drn2_a_18':
        model = models.drn2_a_18(
            pretrained=False,
            num_classes=n_classes,
            scale=args.scale,
            rec_cfg=rec_cfg)
    ...
    for idx, (org_img, img) in enumerate(test_loader):
        ...
        if args.with_rec:
            write_result_as_txt(
                image_name, bboxes, 'outputs/ownTxt/drn18_sra_rec', words)
        else:
            write_result_as_txt(image_name, bboxes, 'outputs/ownTxt/drn18_sra/')

        # visualize results
        if args.vis:
            vis_root = 'outputs/vis/vis_own/'
    ...
    if args.with_rec:
        eval_cmd = 'cd %s;zip -j %s %s/*' % (
            './outputs/ownTxt/', 'drn18_sra_rec.zip', 'drn18_sra_rec')
    else:
        eval_cmd = 'cd %s;zip -j %s %s/*' % (
            './outputs/ownTxt/', 'drn18_sra.zip', 'drn18_sra')
    print(eval_cmd)
    sys.stdout.flush()
    _ = os.popen(eval_cmd)
    _.read()
