...
class Encoder(nn.Module):
    ...
    def __init__(self, hidden_dim, voc, char2id, id2char):
        ...
        self.att2 = MultiHeadAttentionLayer2(self.hidden_dim, 8)
        # self.att2 = MultiHeadAttentionLayer_Fast(self.hidden_dim, 8)
    ...
    def forward(self, x):
        ...
        multi_feature, _ = self.att2(emb_st, x_flatten, x_flatten)
        return holistic_feature + multi_feature
...
class MultiHeadAttentionLayer2(nn.Module):
    def __init__(self, hidden_dim, n_heads, dropout=0.1):
        ...
        self.sr = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=1, stride=1)
        # self.sr = nn.MaxPool2d(kernel_size=3, stride=1, padding=1)
        self.act = nn.GELU()
    ...
    def forward(self, q, k, v, mask=None):
        ...
        q = self.fc_q(q)
        k = k.permute(0, 2, 1).reshape(batch_size, c, 16, 16)
        k = self.sr(k).reshape(batch_size, c, -1).permute(0, 2, 1)
        k = self.layer_norm(k)
        k = self.act(k)
        k = self.fc_k(k)
        v = v.permute(0, 2, 1).reshape(batch_size, c, 16, 16)
        v = self.sr(v).reshape(batch_size, c, -1).permute(0, 2, 1)
        v = self.layer_norm(v)
        v = self.act(v)
        v = self.fc_v(v)
