...
def test(args):
    ...
    if args.arch == 'resnet18':
        ...
        model = models.drn_a_18(
            ...
            scale=args.scale,
            rec_cfg=rec_cfg)
    elif args.arch == 'drn_a_50':
        model = models.drn_a_50(
            pretrained=False,
            num_classes=n_classes,
            scale=args.scale,
            rec_cfg=rec_cfg)
    ...
    for idx, (org_img, img) in enumerate(test_loader):
        ...
        if args.with_rec:
            write_result_as_txt(
                image_name, bboxes, 'outputs/resnet50_ic15_rec/', words)
        else:
            write_result_as_txt(image_name, bboxes, 'outputs/drn18_ic15/')
    ...
    if args.with_rec:
        eval_cmd = 'cd %s;zip -j %s %s/*' % (
            './outputs/', 'resnet50_ic15_rec.zip', 'resnet50_ic15_rec')
    else:
        eval_cmd = 'cd %s;zip -j %s %s/*' % (
            './outputs/', 'drn18_ic15.zip', 'drn18_ic15')
    print(eval_cmd)
    sys.stdout.flush()
    _ = os.popen(eval_cmd)
    _.read()
